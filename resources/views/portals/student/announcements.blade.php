<x-layouts.app title="Announcements">
    <x-ui.page-header
        title="Announcements"
        description="Notices from your school. Anything aimed at one class only reaches that class."
    />

    @if ($announcements->isEmpty())
        <x-ui.card>
            <x-ui.empty-state icon="✦" title="Nothing announced" description="Notices for students appear here." />
        </x-ui.card>
    @else
        <div class="space-y-4">
                ...
                <x-ui.card>
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div class="min-w-0">
                            ...
                            <p class="mt-0.5 text-xs text-slate-500">{{ $announcement->published_at?->format('j F Y, H:i') }}</p>
                        </div>
