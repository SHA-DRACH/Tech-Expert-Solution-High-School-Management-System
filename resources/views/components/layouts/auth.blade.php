...
@include('partials.splash')
    <div class="w-full max-w-md">
        <a href="{{ route('home') }}" class="mb-8 flex items-center justify-center gap-3">
            ...
            @endif
            <span>
                <strong class="block text-sm font-semibold">{{ $school?->name ?? config('app.product', 'NovaxSuites') }}</strong>
        ...
        <p class="mt-6 text-center text-xs text-slate-500">
            Powered by {{ config('app.product', 'NovaxSuites') }}
            <span class="mt-0.5 block">Built by Shadrach Jimice Jr.</span>
        </p>
    </div>
